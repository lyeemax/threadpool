...
public:
    ...
    static inline process_t getCurrentProcessID() {
        return ::getpid();
    }
    ...
    static inline Priority getProcessPriority(process_t id) {
        return convertPriority(
                (PriorityOS)::getpriority(PRIO_PROCESS, id));
    }
    ...
    static inline Priority getCurrentProcessPriority() {
        return getProcessPriority(getCurrentProcessID());
    }
...
{
public:
    ThreadPool(size_t threads = 0, bool if_set_cpu_affinity = true, bool if_set_highest_priority = true):stop(false)
    {
        ...
        for (size_t i = 0; i < threads; ++i)
        {
            workers.emplace_back(
                    [this, number_of_cpus, if_set_highest_priority]
                    {
                        ...
                        {
                            Process::setCurrentProcessPriority(Process::Priority::REALTIME);
...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        res.push_back(threadPool.commit_task(std::bind(fs[i],i)));
    }
    for (int i = 0; i < 100; ++i) {
